...
ZEND_METHOD(reflection_function, getAnnotation)
{
	...
	if (intern->ce == fptr->common.scope && fptr->type == ZEND_USER_FUNCTION && fptr->op_array.annotations
		...
		zend_create_annotation(return_value, *annotation, NULL TSRMLS_CC);
	}   
...
ZEND_METHOD(reflection_class, getAnnotation)
{
	...
	if (filter & ANNOTATION_DECLARED && ce->type == ZEND_USER_CLASS && ce->annotations && zend_hash_find(ce->annotations, name, name_length +1, (void **) &annotation_ref_ref) == SUCCESS) {
		zend_create_annotation(return_value, *annotation_ref_ref, NULL TSRMLS_CC);
		return;
	}
...
ZEND_API zend_class_entry *reflection_get_annotation_ce(const char *aname, const uint alength TSRMLS_DC)
{
	zend_class_entry *ace = zend_fetch_class(aname, alength, ZEND_FETCH_CLASS_AUTO TSRMLS_CC);
	if (!ace) { 
...
PHP_MINIT_FUNCTION(reflection) /* {{{ */
{
	...
	zend_get_annotation_ce = reflection_get_annotation_ce;
