...
ZEND_METHOD(reflection_class, getAnnotations)
{
	...
	if (filter & ANNOTATION_DECLARED && ce->type == ZEND_USER_CLASS && ce->annotations) {
		reflection_add_declared_annotations(return_value, ce->annotations, getThis() TSRMLS_CC);
	}

	if (filter & ANNOTATION_INHERITED) {
		while((ce = ce->parent)) {
			if (ce->type == ZEND_USER_CLASS && ce->annotations) {
				reflection_add_inherited_annotations(return_value, ce->annotations, getThis() TSRMLS_CC);
			}
		}
	}
}
...
ZEND_METHOD(reflection_class, getAnnotation)
{
	...
	if (filter & ANNOTATION_INHERITED) {
		while((ce = ce->parent)) {
			if (ce->type == ZEND_USER_CLASS && ce->annotations && reflection_get_inherited_annotation(ce->annotations, name, name_length, getThis(), return_value TSRMLS_CC) == SUCCESS) {
				return;
			}
		}
	}
...
ZEND_METHOD(reflection_class, hasAnnotation)
{
	...
	if (filter & ANNOTATION_INHERITED) {
		while((ce = ce->parent)) {
			if (ce->type == ZEND_USER_CLASS && ce->annotations && reflection_get_inherited_annotation(ce->annotations, name, name_length, getThis(), NULL TSRMLS_CC) == SUCCESS) {
				RETURN_TRUE;	
			}
		}
	}
